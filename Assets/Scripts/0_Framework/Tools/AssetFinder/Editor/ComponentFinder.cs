namespace DGFramework.Editor.Tools.AssetFinder
{
    ...
    public class ComponentFinder : EditorWindow
    {
        ...
        {
            Debug.Log($"Find : {typeName}");
            if (string.IsNullOrEmpty(typeName))
            {
                Debug.LogWarning("Component type cannot be empty");
                return;
            }
            ...
            if (type == null)
            {
                Debug.LogError($"Component type '{typeName}' not found. Try using full namespace (e.g., UnityEngine.UI.Text).");
                return;
            }
